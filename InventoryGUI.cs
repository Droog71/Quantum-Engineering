    ...
    public void OnGUI()
    {
        ...
        if (playerController.stateManager.worldLoaded == true && GetComponent<MainMenu>().finishedLoading == true)
        {
            if (playerController.inventoryOpen == true)
            {
                ...
                GUI.DrawTexture(gc.inventoryBackgroundRect, td["Container Background"]);
                ...
                foreach (InventorySlot slot in playerInventory.inventory)
                {
                    ...
                    {
                        GUI.DrawTexture(gc.inventorySlotRects[inventorySlotNumber], td[slot.typeInSlot]);
                    }
                ...
                {
                    GUI.DrawTexture(gc.inventoryInfoRectBG, td["Interface Background"]);
                    GUI.Label(gc.inventoryInfoRect, "\nLeft Shift + Click & Drag to split stack.\n\nLeft Control + Click to transfer entire stack.");
                    GUI.DrawTexture(gc.storageInventoryBackgroundRect, td["Container Background"]);
                    ...
                    foreach (InventorySlot slot in playerController.storageInventory.inventory)
                    {
                        if (!slot.typeInSlot.Equals("") && !slot.typeInSlot.Equals("nothing"))
                        {
                            GUI.DrawTexture(gc.storageInventorySlotRects[storageInventorySlotNumber], td[slot.typeInSlot]);
                        }
                        if (slot.amountInSlot != 0)
                        {
                ...
                if (playerController.storageGUIopen == true) //PLAYER IS ACCESSING A STORAGE CONTAINER
                {
                    ...
                    {
                        GUI.DrawTexture(new Rect(Event.current.mousePosition.x - ScreenWidth * 0.0145f, Event.current.mousePosition.y - ScreenHeight * 0.03f, (ScreenWidth * 0.029f), (ScreenHeight * 0.06f)), td[playerController.itemToDrag]);
                ...
                else
                {
                    ...
                    {
                        //DROPPING ITEMS INTO THE PLAYER'S INVENTORY
                        GUI.DrawTexture(new Rect(Event.current.mousePosition.x - ScreenWidth * 0.0145f, Event.current.mousePosition.y - ScreenHeight * 0.03f, (ScreenWidth * 0.025f), (ScreenHeight * 0.05f)), td[playerController.itemToDrag]);
                        if (Input.GetKeyUp(KeyCode.Mouse0))
                        {
                            playerController.draggingItem = false;
                            int inventoryDropSlot = 0;
                ...
                if (playerController.craftingGUIopen == true)
                {
                    if (craftingPage == 0)
                    {
                        ...
                        {
                            GUI.DrawTexture(gc.craftingInfoBackgroundRect, td["Interface Background"]);
                        ...
                        GUI.DrawTexture(gc.craftingBackgroundRect, td["Interface Background"]);
